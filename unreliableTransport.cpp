...
unreliableTransportC::unreliableTransportC(const std::string &hostname,
  ...
  if (status != 0) {
    FATAL << "Error looking up IPv4 Address for " << hostname << "error #"
          << status << ENDL;
  ...
  DEBUG << "Found IPv4 Address for " << hostname << " as "
        << inet_ntoa(((struct sockaddr_in *)result->ai_addr)->sin_addr) << ENDL;
  ...
  DEBUG << "Found IPv4 Address for " << hostname << " as "
        << inet_ntoa(serverAddr.sin_addr) << ENDL;
...
void unreliableTransportC::udt_send(const datagramS &data) const {
  ...
  DEBUG << "Sending datagram to " << inet_ntoa(serverAddr.sin_addr) << ":"
        << ntohs(serverAddr.sin_port) << ENDL;
  TRACE << "Sending: " << toString(data) << ENDL;
  ssize_t bytesSent =
      sendto(socketFd, &data, sizeof(datagramS), 0,
...
ssize_t unreliableTransportC::udt_receive(const datagramS &data) const {
  ...
  if (FD_ISSET(socketFd, &read_fds)) {
    ...
    bytesRead = recvfrom(socketFd, (void *)&data, sizeof(datagramS), 0, nullptr,
                         nullptr);
    TRACE << "Received " << bytesRead << " bytes." << ENDL;
    if (bytesRead == -1) {
      FATAL << "Error when calling recvfrom() throwing error #" << errno
            << ENDL;
      close(socketFd);
      throw(
